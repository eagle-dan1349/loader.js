...
(function (global) {
  ...
  function missingModule(name, referrer) {
    const error = new Error('Could not find module `' + name + '` imported from `' + referrer + '`');
    console.error(error);
    throw error;
  }

  function __normalize(name, referrer) {
    if (name.charAt(0) !== '/') {
  ...
  function findModule(name, referrer, pending) {
    name = __normalize(name, referrer);
    stats.findModule++;
  ...
  requirejs.unsee = function (moduleName) {
    findModule(moduleName, '(unsee)', false)
      .unsee();
  };
